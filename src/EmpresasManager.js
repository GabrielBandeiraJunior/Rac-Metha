...
const EmpresasManager = () => {
  ...
  const fetchEmpresas = async () => {
    try {
      const response = await axios.get('https://process.env.REACT_APP_API_URL/empresas');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingEmpresa) {
        await axios.put(`https://process.env.REACT_APP_API_URL/empresas/${editingEmpresa.id}`, formData);
      } else {
        await axios.post('https://process.env.REACT_APP_API_URL/empresas', formData);
      }
      fetchEmpresas();
      resetForm();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Tem certeza que deseja deletar esta empresa?')) {
      try {
        await axios.delete(`https://process.env.REACT_APP_API_URL/empresas/${id}`);
        fetchEmpresas();
      } catch (error) {
